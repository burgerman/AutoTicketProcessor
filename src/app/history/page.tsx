...
export default function HistoryPage() {
  ...
  const [selectedTicket, setSelectedTicket] = useState<ExtractedTicketData | null>(null);
        ...
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
          <Card className="max-w-4xl mx-auto">
            <CardHeader>
              ...
              <CardDescription>A list of tickets you have processed in this session. Click on a ticket to view details.</CardDescription>
            </CardHeader>
                  ...
                  {processedTickets.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {processedTickets.map((ticket, index) => (
                        <DialogTrigger asChild key={index} onClick={() => setSelectedTicket(ticket)}>
        ...
        {selectedTicket && (
            <DialogContent className="sm:max-w-xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Full details for the processed ticket.
                    </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    <div className="flex flex-wrap gap-x-6 gap-y-3 text-sm">
                        <div className="flex items-center gap-2" title="Affected Service">
                            <Server className="h-4 w-4 text-primary" />
                            <Badge variant="secondary">{selectedTicket.affectedService}</Badge>
                        </div>
                        <div className="flex items-center gap-2" title="Issue Type">
                            <AlertTriangle className="h-4 w-4 text-primary" />
                            <Badge variant="secondary">{selectedTicket.issueType}</Badge>
                        </div>
                        <div className="flex items-center gap-2" title="Priority">
                    ...
                    <Card className="mt-4">
                      <CardHeader>
                        <CardTitle className="text-sm font-medium">Original Ticket Text</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <p className="text-sm text-muted-foreground whitespace-pre-wrap">{selectedTicket.ticketText}</p>
                      </CardContent>
                    </Card>
                </div>
                 <DialogClose asChild>
